...
unsigned int frequence = 100; // Define a frequência (ou delay) entre operações de LED.
...
void gpioSetup(){
	...
	HWREG(CM_PER_GPIO1_CLKCTRL) = 0x40002;  //ativa o clock do gpio1, pagina 1284

	// ==
	//HWREG(CM_PER_GPIO1_CLKCTRL) |= 0x40000; // bit 18 setado
	//HWREG(CM_PER_GPIO1_CLKCTRL) |= 0x2; // ativa(enable)
...
void butConfig ( ){
	...
	/*0010 0000 | (1<<5) modo entrada
	  ...
	  0010 1111
	    2   F
	  */
	/*Configura a multiplexação do pino 12 do GPIO1 para ser usado como entrada*/

	HWREG(CM_PER_GPMCAD14_REGS) |= 0X2F; /*Configura a multiplexação do pino 14 do 
	GPIO1 para ser usado como entrada*/
    
    HWREG(GPIO1_OE) |= 1<<12; /*configura o pino 12 do gpio1 como entrada*/
	HWREG(GPIO1_OE) |= 1<<14; /*configura o pino 14 do gpio1 como entrada*/

    /* Setting interrupt GPIO pin. */
	HWREG(GPIO1_IRQSTATUS_SET_0) |= 1<<12; /*Habilita a interrupção para o pino 12*/
	HWREG(GPIO1_IRQSTATUS_SET_1) |= 1<<14; /*Habilita a interrupção para o pino 14*/
	// associar um pino a determinado grupo de interrupção

  	/* Enable interrupt generation on detection of a rising edge.*/
	HWREG(GPIO1_RISINGDETECT) |= 1<<12;	/*habilita a detecção de borda de subida
	no pino 12 do gpio1*/

	HWREG(GPIO1_RISINGDETECT) |= 1<<14;	/*habilita a detecção de borda de subida
	no pino 14 do gpio1*/
...
void gpioIsrHandler(unsigned int irq_number){
	...
	switch (irq_number){
		case 95:
			timerIrqHandler(); /*A função timerIrqHandler é definida em outra parte do código 
			e trata de limpar o status da interrupção do timer e desabilitar o 
			timer*/
		break;
		case 98:
			...
			putString("button A press!\n\r",17); /*Exibe uma mensagem indicando que o botão A 
		...
		case 99:
			...
			if(frequence > 1000){
				...
				frequence += 200;
			}/* Se a variável frequence é maior que 1000, ela é redefinida para 100. Caso contrário, 
			200 é adicionado à frequence. Isso pode ser usado para ajustar a frequência de algum 
			processo ou temporizador em resposta à pressão do botão B.
			Na prática, pressionar o botão B altera a frequência de piscamento dos LEDs. A variável 
			frequence é usada para definir o intervalo de tempo entre o acendimento e o apagamento 
			dos LEDs. O comportamento é ajustado conforme a frequência é incrementada em resposta 
			às pressões do botão e redefinida quando excede um certo limite. Isso cria um efeito 
			de mudança na taxa de piscamento dos LEDs, permitindo ao usuário ver claramente a 
			alteração na frequência de operação.*/
		break;
		default:
		break;
	}
...
void ISR_Handler(void){
	...
	gpioIsrHandler(irq_number);/* é chamada com o número da interrupção (irq_number) como 
	argumento. Isso permite que a ISR trate a interrupção de acordo com o número da IRQ 
	recebido, realizando as ações necessárias para processar o evento que causou a 
	interrupção.*/
